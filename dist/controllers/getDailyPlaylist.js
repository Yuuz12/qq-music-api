...
function extractPicurl(item) {
    const imgMatch = item.match(/<img[^>]+class="[^"]*playlist__pic[^"]*"[^>]*src="([^"]+)"/) ||
        item.match(/<img[^>]*src="([^"]+)"/);
...
exports.default = async (ctx) => {
    ...
    try {
        const res = await axios_1.default.get(PAGE_URL, {
            headers: {
            ...
            timeout: 10000,
            responseType: 'text',
            transformResponse: [(d) => d], // 保持原始 HTML，不交给 axios 默认 JSON 解析
        });
        ...
        for (const item of items) {
            const nameMatch = item.match(/<h3[^>]*class="[^"]*playlist__name[^"]*"[^>]*>[\s\S]*?>([^<]*)<\/a>\s*<\/h3>/);
