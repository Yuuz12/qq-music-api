...
export const apiExplorerOverrides: Record<string, Partial<ApiExplorerEndpoint>> = {
  ...
  getUserProfile: {
    id: 'get-user-profile',
    description:
      'Get user profile aggregation: nick/avatar/fans/following/我喜欢/custom playlists. Empty uin = current login user; pass another user encrypt uin (EncUin) to read their homepage.',
    queryParams: [
      {
        key: 'uin',
        label: 'Target EncUin',
  ...
  getPlayRecently: {
    ...
    description:
      'Get cloud play-recently list (最近播放, client-native sync interface music.musicasset.PlayRecentlyRead.GetPlayRecentlyInfo). type=2 returns up to 500 songs (track in new format + lastTime/listenCnt/lastPlayDevice), 3=albums, 4=playlists, 1=all-summary. Requires cookie; sign not required.',
    queryParams: [
      {
        key: 'type',
        label: 'Type',
  ...
  reportPlayRecently: {
    ...
    description:
      'Report one play to cloud recently-played (client-same music.musicasset.PlayRecentlyWrite.ReportPlayRecentlyInfo: listenCnt increment +1, lastTime = now). Requires cookie. response.data.innerCode === 0 means written; the cloud list on all devices updates immediately.',
  ...
  deletePlayRecently: {
    ...
    description:
      'Delete one song from cloud recently-played (music.musicasset.PlayRecentlyWrite.DeletePlayRecentlyInfo). Requires cookie. response.data.innerCode === 0 means deleted; removal syncs to all devices.',
    bodyDescription: 'JSON body with songId.',
    bodyExample: {
      songId: 301964992,
    },
  ...
  userRefresh: {
    id: 'user-refresh',
    description:
      'Refresh login (延长登录有效期, ported from jsososo /user/refresh): exchange current musickey for a new one via QQConnectLogin.LoginServer/QQLogin. Requires credential headers. Returns new musickey in response.data; code 1000 means the key has expired and re-login (QR scan) is required.',
  },
  getRecommendFeed: {
    id: 'get-recommend-feed',
    description:
      'Personalized home recommend feed (为你推荐, same upstream as the QQ Music App recommend tab: music.recommend.RecommendFeed/get_recommend_feed). Returns playlist cards incl. 每日30首 (type=daily) and taste-based playlists with recommendation reason. With credential headers the content is personalized; without, generic hot playlists are returned.',
  },
  addDislike: {
    id: 'add-dislike',
    description:
      '删除这首歌曲（不喜欢）：写入黑名单（客户端同款 user.FeedbackBlack/AddBlackList，需登录凭据）。response.data.innerCode === 0 表示成功。',
    bodyDescription: 'JSON body with songId.',
  ...
  cancelDislike: {
    id: 'cancel-dislike',
    description:
      '取消不喜欢：从黑名单移除（客户端同款 user.FeedbackBlack/CancelBlackList，需登录凭据）。response.data.innerCode === 0 表示成功。',
    bodyDescription: 'JSON body with songId.',
    bodyExample: { songId: 301964992 },
  },
  getDislikeList: {
    id: 'get-dislike-list',
    description:
      '不喜欢（黑名单）列表：客户端「删除这首歌曲」功能的历史记录查询（需登录凭据），返回 response.data 黑名单结果。',
  },
};
