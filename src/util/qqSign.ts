/**
 * QQ 音乐 musics.fcg 请求签名（zzc，算法版本 __sign_hash_20200305）。
 *
 * 来源：jsososo/QQMusicApi 的 util/sign.js，原样内嵌（混淆 VM 字节码，不做改动）。
 * 2026-08 实测：上游 u6.y.qq.com 网关仍接受该签名（顶层 code=0，模块正常执行）。
 * 输出含随机成分（同一输入两次签名不同），属上游原设计，勿做确定性断言。
 */
// biome-ignore-all lint/suspicious/noExplicitAny: 原样内嵌的混淆实现，不声明类型
const SIGN_MODULE_SOURCE = `module.exports = (obj) => {
  let str = obj;
...
type SignFn = (data: unknown) => string;

let cached: SignFn | null = null;

/** 隔离执行内嵌模块（CommonJS 风格），拿到 module.exports 并缓存 */
function loadSign(): SignFn {
  if (!cached) {
    const mod = { exports: {} as Record<string, unknown> };
    const factory = new Function('module', `${SIGN_MODULE_SOURCE}\nreturn module.exports;`) as (
      m: unknown,
    ) => unknown;
    cached = factory(mod) as SignFn;
  }
  return cached;
}

/** 对请求 data（对象或字符串）计算 zzc 签名，如 zzaxxxxxx...（48 位） */
export default function qqSign(data: unknown): string {
  return loadSign()(data);
}
