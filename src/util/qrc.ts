...
function bitNum(a: Buffer, b: number, c: number): number {
...
function initialPermutation(inputData: Buffer): [number, number] {
  const p = (list: number[]): number => {
    let v = 0;
    for (let i = 0; i < 32; i++) v |= bitNum(inputData, list[i], 31 - i);
    return v >>> 0;
  };
  return [
    p([
...
function inversePermutation(s0: number, s1: number): Buffer {
  const data = Buffer.alloc(8);
  const byte = (s1bits: number[], s0bits: number[]): number => {
    let v = 0;
    for (let k = 0; k < 8; k++) {
...
function fRound(state: number, key: number[]): number {
  const t1 =
...
function keySchedule(key8: Buffer, decrypt: boolean): number[][] {
...
function desCrypt(input8: Buffer, schedule: number[][]): Buffer {
...
function funcDes(buff: Buffer, key8: Buffer, decrypt: boolean): Buffer {
 ...
 */
export function qrcDecryptHex(hex: string): string {
  if (!hex) return '';
 ...
 * 返回空串表示未找到。
 */
function extractLyricContent(xml: string): string {
  const s = String(xml);
 ...
 */
export function qrcXmlToLrc(xml: string): string {
 ...
 */
export function qrcXmlToWordData(xml: string): {
  lrc: string;
  words: Record<string, { text: string; start: number; end: number }[]>;
} {
  const content = extractLyricContent(xml);
  const out: Record<string, { text: string; start: number; end: number }[]> = {};
  const lines: string[] = [];
  if (!content) return { lrc: lines.join('\n'), words: out };
  for (const raw of content.split('\n')) {
    const t = raw.match(/^\[(\d+),(\d+)\]/);
    if (!t) continue;
    ...
    const spans: { text: string; start: number; dur: number }[] = [];
    ...
    for (let i = 0; i < spans.length; i++) {
      ...
      if (raw === '') continue; // 空串词丢弃；纯空格词保留（词间空格可见）
      const end =
        i + 1 < spans.length
