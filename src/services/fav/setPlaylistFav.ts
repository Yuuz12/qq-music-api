 ...
 *
 * 2026-08 新增（本播放器项目适配）：
 * 「收藏歌单」按钮真实接口为 music.musicasset.PlaylistFavWrite
 * （method=FavPlaylist 收藏 / CancelFavPlaylist 取消收藏，
 *  param={ uin, v_playlistId:[disstid] }，成功判据 req_1.code === 0），
 * 逆向自 QQ 音乐 Web 端歌单收藏模块。
 * 与「喜欢」歌曲/收藏专辑相同：该写接口在登录态下强制要求官方前端加密
 ...
 * 常驻 headless 浏览器或 Electron 隐藏窗口），转发到代理的 /playlist-fav-write 端点，
 * 由官方请求库完成 签名→加密→发送→解密。
 *
 * 多用户（2026-08）：把当前请求的凭据（cookie/uin）一并转发给代理，
 * 代理按 cookie 切换页面登录态，确保写入的是**用户自己**的收藏歌单。
 *
 * 环境变量 PROXY_URL 可覆盖代理地址（默认 http://localhost:9339）。
 */

interface SetPlaylistFavParams {
  /** 歌单数字 id（disstid） */
  disstid?: string | number;
  /** 当前是否已收藏：true 时调用 CancelFavPlaylist（取消收藏），false 时调用 FavPlaylist（收藏） */
  isFan?: boolean;
}

const PROXY_URL = process.env.PROXY_URL || 'http://localhost:9339';

export default async ({ disstid = '', isFan = false }: SetPlaylistFavParams = {}) => {
  ...
  try {
    ...
    const res = await axios.post(`${PROXY_URL}/playlist-fav-write`, payload, {
