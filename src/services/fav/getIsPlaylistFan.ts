 ...
 *
 * 2026-08 新增（本播放器项目适配）：
 * 逆向自 QQ 音乐 Web 端歌单收藏模块运行时代码：
 * - module: music.musicasset.PlaylistFavRead
 * - method: IsPlaylistFan
 * - param:  { v_tid: [disstid, ...] }（⚠️ 必须是数字 id：传字符串上游返回 10006 参数错误）
...
interface GetIsPlaylistFanParams {
  /** 歌单数字 id（disstid）列表 */
  disstids?: (string | number)[];
}

export interface GetIsPlaylistFanResponse {
  /** 0 成功；上游业务码（如 1000 未登录）原样透出；服务层失败为 -1 */
  code: number;
  /** 与 musicu.fcg 单请求结构对齐：data.m_fan = { [disstid]: 1 | 0 }（多批已合并） */
...
type GetIsPlaylistFanResult = Promise<{
  status: number;
  body: { response: GetIsPlaylistFanResponse };
}>;

export default async ({
  disstids = [],
}: GetIsPlaylistFanParams = {}): Promise<GetIsPlaylistFanResult> => {
  ...
  logServiceRequest('getIsPlaylistFan', UPSTREAM_URL, {
    count: tids.length,
    ...
    batches.map((batch) => {
      const data = {
        ...
        req_0: {
          ...
          method: 'IsPlaylistFan',
          param: {
            // 上游要求数字 id：传字符串返回 code 10006 参数错误（2026-08-23 实测）
            v_tid: batch.map(Number),
          },
        },
      };
  ...
  if (!okBatches) {
    logServiceFailure('getIsPlaylistFan', UPSTREAM_URL, firstError, { count: tids.length });
    return {
      status: 500,
      body: {
        response: {
  ...
  // 部分批次失败：降级返回已取得的部分（缺失的歌单保持无收藏态），同时记录失败日志
  if (okBatches < batches.length) {
    logServiceFailure('getIsPlaylistFan', UPSTREAM_URL, firstError, {
      count: tids.length,
      okBatches,
  ...
  logServiceSuccess('getIsPlaylistFan', UPSTREAM_URL, {
    code: response.code,
